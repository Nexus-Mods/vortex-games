...
async function addConfigFiles(api: types.IExtensionApi, profileId: string, files: IFileEntry[]): Promise<void> {
  if (files.length === 0) {
    return;
  }
...
async function addRegularFiles(api: types.IExtensionApi, files: IFileEntry[]): Promise<void> {
  ...
  for (const entry of files) {
    ...
    const mod = util.getSafe(state.persistent.mods, [GAME_ID, candidateId], undefined) as types.IMod | undefined;
