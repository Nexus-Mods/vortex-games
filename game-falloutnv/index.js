...
// extensions/games/game-falloutnv/index.ts
...
var GAME_ID = "falloutnv";
var NEXUS_DOMAIN_NAME = "newvegas";
var STEAMAPP_ID = "22380";
var STEAMAPP_ID2 = "22490";
var GOG_ID = "1454587428";
var MS_ID = "BethesdaSoftworks.FalloutNewVegas";
var EPIC_ID = "5daeb974a22a435988892319b3a4f476";
var PATCH_4GB_EXECUTABLES = ["FNVpatch.exe", "FalloutNVpatch.exe", "Patcher.exe"];
...
async function findGame() {
  const storeGames = await import_vortex_api.util.GameStoreHelper.find(gameStoreIds).catch(() => []);
  if (!storeGames.length) return;
  if (storeGames.length > 1) (0, import_vortex_api.log)("debug", "Mutliple copies of New Vegas found", storeGames.map((s) => s.gameStoreId));
  const selectedGame = storeGames[0];
  if (["epic", "xbox"].includes(selectedGame.gameStoreId)) {
    try {
      const folders = await import_vortex_api.fs.readdirAsync(selectedGame.gamePath).filter((p) => !import_path.default.extname(p) && !p.startsWith("."));
      ...
      if (!availableLocales.length) {
        (0, import_vortex_api.log)("warn", "Could not find any recognised locale folders for New Vegas", { folders, path: selectedGame.gamePath });
        selectedGame.gamePath = import_path.default.join(selectedGame.gamePath, folders[0]);
        selectedLanguage = folders[0].toUpperCase().replace("Fallout New Vegas", "").trim();
      } else if (availableLocales.length === 1) selectedGame.gamePath = import_path.default.join(selectedGame.gamePath, localeFoldersXbox[availableLocales[0]]);
      else {
        const selectedLocale = availableLocales.includes("en") ? "en" : availableLocales[0];
        selectedLanguage = selectedLocale.toUpperCase();
        multipleLanguages = true;
        (0, import_vortex_api.log)("debug", `Defaulting to the ${selectedLocale} game version`, { store: selectedGame.gameStoreId, folder: localeFoldersXbox[selectedLocale] });
        selectedGame.gamePath = import_path.default.join(selectedGame.gamePath, localeFoldersXbox[selectedLocale]);
      }
    } catch (err) {
      (0, import_vortex_api.log)("warn", "Could not check for Fallout NV locale paths", err);
    }
  }
  return selectedGame;
}
...
function prepareForModding(api, discovery) {
  const gameName = import_vortex_api.util.getGame(GAME_ID)?.name || "This game";
...
async function requiresLauncher(gamePath, store) {
  ...
  try {
    const game = await import_vortex_api.util.GameStoreHelper.findByAppId([MS_ID], "xbox");
    const normalizeFunc = await import_vortex_api.util.getNormalizeFunc(gamePath);
...
function testInstaller4GBPatch(api) {
  return (files, gameId, archivePath, details) => {
    ...
    const gameMode = import_vortex_api.selectors.activeGameId(state);
...
function main(context) {
  ...
  context.registerInstaller(
    "falloutnv-4gb-patch",
    25,
    testInstaller4GBPatch(context.api),
    applyInstaller4GBPatch(context.api)
  );
  return true;
}
module.exports = {
