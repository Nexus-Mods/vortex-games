    ...
    return Promise.reduce(Array.from(uniqueCandidates), (accum, mod) => {
      ...
      return fs.statAsync(path.join(modFolder))
        .then(() => new Promise(async (resolve) => {
          ...
          const mapped = await Promise.map(candidates,
            cand => fs.statAsync(cand.filePath)
  ...
  return Promise.reduce(mods, (accum, mod) => findModFolder(installationPath, mod)
    .then(modName => {
      if (modName === undefined) {
        // The mod is not installed correctly, or perhaps
        //  it's a modType we don't cater for.
        return Promise.resolve(accum);
      }
    ...
    React.createElement('div', {
      ...
      React.createElement('ul', {},
        ...
        React.createElement('li', {}, t('You can change the way the priorities are assgined using the "Switch To Position/Prefix based" button. '
          ...
          + 'restrict the priorities to the number of load order entries that are available.', { ns: I18N_NAMESPACE })),
